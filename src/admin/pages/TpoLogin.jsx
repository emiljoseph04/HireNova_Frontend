...
import { tpoLoginAPI } from "../../services/allAPI";
...
function TpoLogin() {
  ...
  const handleLogin = async () => {
    ...
    if (!email || !password) {
      ...
      try {
        const result = await tpoLoginAPI(details);
        ...
        if (result.status === 200) {
          ...
          if (result.data.existingUser.role === "tpo") {
    ...
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: "#501558" }}>
      ...
      <div className="pt-32 pb-20 px-6 flex justify-center">
        <div className="bg-white border shadow-2xl rounded-2xl p-10 w-full max-w-md" style={{ borderColor: "#501558" }}>
          <h1 className="text-3xl font-extrabold text-center" style={{ color: "#501558" }}>TPO Login</h1>
          <p className="text-center text-gray-600 mt-2">Only authorized TPOs can access this panel.</p>

          <div className="mt-8 space-y-5">
            <input type="email" placeholder="TPO Email" value={details.email} onChange={e => setDetails({ ...details, email: e.target.value })} className="w-full p-3 border rounded-lg bg-white outline-none focus:ring-2" style={{ borderColor: "#501558" }} />
